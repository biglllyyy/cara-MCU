...
static const U8 kTable[] = {KEY_TABLE};
static U8 expkey[4 * Nb * (Nr + 1)];
static const U8 key[]={KEY_TABLE};

/* 自动初始化AES */
static U8 aesInit = 0;

static const U8 Sbox[256] = {		// forward s-box
...
static const U8 InvSbox[256] = {	// inverse s-box
...
static const U8 Xtime2Sbox[256] = {
...
static const U8 Xtime3Sbox[256] = {
...
//解码
static void Decrypt (U8 *in, U8 *expkey, U8 *out)
{
	int idx, round;
	U8 state[Nb * 4];
	...
	AddRoundKey((U32*)state, (U32*)expkey + Nr * Nb);
	round = Nr;
	...
	while( round-- )
	{
		AddRoundKey (( U32 *)state, ( U32 *)expkey + round * Nb);
		if( round )
...
static void ExpandKey (const U8 *key, U8 *expkey)
{
	const U8 Rcon[11] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
...
// encrypt one 128 bit block，加密
static void Encrypt (const U8 *in, U8 *expkey, U8 *out)
{
	U32 round, idx;
	U8 state[Nb * 4];
	/* get input data */
	...
	AddRoundKey (( U32 *)state, ( U32 *)expkey);
...
{
	ExpandKey (kTable, expkey);
}

void aes_decrypt(U8 *data)
{
	if (!aesInit) aes_encrypt_init();
	Decrypt (data, expkey, data);
}
