...
static Bool chord_pwm_first=TRUE;
#if 0
...
U8 PPGConfig_ChannelControl_Stop(PPGUtil* a_pPPGUtil)
{
    U16 data;
    if (a_pPPGUtil==0)
        return FALSE;
...
U8 PPGConfig_ChannelControl_Start(PPGUtil* a_pPPGUtil)
{
    U16 data;
    if (a_pPPGUtil==0)
        return FALSE;
...
U8 PPGConfig_ChannelPCSR(PPGUtil* a_pPPGUtil, U16 a_nCycle)
{
    if (a_pPPGUtil==0)
        return FALSE;
...
U8 PPGConfig_ChannelPDUT(PPGUtil* a_pPPGUtil, U16 a_pdutRes)
{
    if (a_pPPGUtil==0)
        return FALSE;
...
** \input
				...
				duty: 占空比设置（0.1）
*/
void HalPwmStart(PPGUtil *ppg_chn,U8 chn,U32 frq,U8 duty)
{
    U16 cycle_value = 0;
    ...
    do
    {
		...
		if(frq > 100000)
        {
            dbg_string("frq too big \n",0);
        }
	else if(frq >= 10000)
        {
            factor_frq_div = PPG_CKS_PCLK;
            cycle_value = (U16)(PPG_PCLK/(frq)-1);
        }	
        else if(frq >= 1000)
        {
        ...
        {
            dbg_string("req %d too samall",frq);
        }
        if(duty > 0xff)
        {
            duty = 0xff;
        ...
        PPGConfig_ChannelPDUT(ppg_chn,cycle_value*duty/0xff);//设置PDUT（占空比）
...
void HalPwmStop(PPGUtil *ppg_chn,PPG_PIN_STS_e sts)
{
    if(ON == ppg_chn->nPPG_OnOff_Sts)
	{
        PPGConfig_ChannelControl(ppg_chn, PPG_PCN_CNTE, 0); //count disable
        PPGConfig_ChannelControl_Stop(ppg_chn);
    }
    PPGDrv_setPinState(ppg_chn,sts);
    chord_pwm_first=TRUE;
}
...
void HalPwmFineStart(PPGUtil *ppg_chn,U8 chn,U32 frq,U16 duty)
{
	...
	if(chord_pwm_first)
	{
		...
		PPGConfig_ChannelControl(ppg_chn, PPG_PCN_OSEL, 0); //Normal Polarity
		PPGConfig_ChannelControl(ppg_chn, PPG_PCN_CNTE, 1); //count enable
        	chord_pwm_first=FALSE;
	}
	ppg_chn->nFrq=frq;
	ppg_chn->nPCSR=cycle_value;
	ppg_chn->nPDUT=cycle_value*duty/0xffff;
    }
    while (0);
}

void HalPwmInit(PPGUtil *ppg_chn,U8 chn)
{
    memset(ppg_chn,0,sizeof(PPGUtil));
    ppg_chn->nCh = chn;
}

PPGUtil l_sPPG_DIM;

void Init_PPG_Test(void)
{
	ICR26 = 42;
	l_sPPG_DIM.nCh = BUZZER_CHN;
	PPGDrv_Init(&l_sPPG_DIM, BUZZER_CHN, DIM_MODE);
	PPGConfig_ChannelPCSR(&l_sPPG_DIM, PCSR_VALUE);
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_STRG, 1); //Active by software only
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_MDSE, 0); //PWM operation
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_RTRG, 1); //restart disable
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_CKS0, PPG_CKS_PCLK);	//set ppg count clock
	//PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_PGMS, 1);	//output mask and the GPIO state is assigned with the state of PPG_PCN_OSEL
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_PGMS, 1); //No output mask
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_EGS0, PPG_EGS_NA);	//No output mask
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_IREN, 0); //Interrupt request disabled
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_IRQF, 0); //Clear Interrupt request flag
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_IRS0, PPG_IRS_Borrow_CounterMatch);	//Don't care
	PPGConfig_ChannelControl(&l_sPPG_DIM, PPG_PCN_OSEL, 0); //Normal Polarity

	//set to ~0% duty cycle
	PPGConfig_ChannelPDUT(&l_sPPG_DIM, PDUT_1_PERCENT*0);//PPG_PCLK/(CountRatio*CarrierFreq_DIM)*1/100);
	//start PPG with assigned setting
	PPGConfig_ChannelControl_Start(&l_sPPG_DIM);
}
