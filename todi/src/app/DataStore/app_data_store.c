...
/* 本次行车里程 */
U8 read_this_trip_distance(U32 *distance)
{
	T_DATA_STORE	*sData;
	sData = get_data_store_ptr();
	if (!sData) return FALSE;
	
	*distance = sData->this_trip_distance;
	return TRUE;
}
U8 write_this_trip_distance(U32 distance)
{
	T_DATA_STORE	*sData;
	sData = get_data_store_ptr();
	if (!sData) return FALSE;

	sData->this_trip_distance = distance;
	mid_save_data((U8*)sData, sizeof(T_DATA_STORE));
	return TRUE;	
}

/* 本次行车耗油 */
U8 read_this_trip_used_fuel(U32 *used_fuel)
{
	T_DATA_STORE	*sData;
	sData = get_data_store_ptr();
	if (!sData) return FALSE;
	
	*used_fuel = sData->this_trip_used_fuel;
	return TRUE;
}
U8 write_this_trip_used_fuel(U32 used_fuel)
{
	T_DATA_STORE	*sData;
	sData = get_data_store_ptr();
	if (!sData) return FALSE;

	sData->this_trip_used_fuel = used_fuel;
	mid_save_data((U8*)sData, sizeof(T_DATA_STORE));
	return TRUE;	
}

/* 总耗油 */
U8 read_total_used_fuel(U32 *used_fuel)
{
	T_DATA_STORE	*sData;
	sData = get_data_store_ptr();
	if (!sData) return FALSE;
	
	*used_fuel = sData->total_used_fuel;
	if(*used_fuel > 50000)
		*used_fuel = 0;
	return TRUE;
}
U8 write_total_used_fuel(U32 used_fuel)
{
	T_DATA_STORE	*sData;
	sData = get_data_store_ptr();
	if (!sData) return FALSE;

	sData->total_used_fuel = used_fuel;
	mid_save_data((U8*)sData, sizeof(T_DATA_STORE));
	return TRUE;	
}
