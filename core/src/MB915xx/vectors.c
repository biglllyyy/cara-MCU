...
__interrupt void AD_Converter_interrupt(void);
...
#pragma	intvect	ext_interrupt_0_7_isr	  16	///< External interrupt 0-7
#pragma	intvect	ext_interrupt_8_15_isr    17	///< External interrupt 8-15  External low-voltage detection interrupt
...
#pragma	intvect	ISR_LIN4_Reception 28	///< Multi-function serial interface ch.4 (reception completed) Multi-function serial interface ch.4 (status)
#pragma	intvect	ISR_LIN4_Transmission 29	///< Multi-function serial interface ch.4 (transmission completed)
...
#pragma	intvect	ISR_FreeRunTimer0 39	///< 16-bit Free-run timer 0 (0 detection) / (compare clear)
#pragma	intvect	ISR_FreeRunTimer1 40	/* freeruntimer1中断向量 */
#pragma	intvect	ISR_FreeRunTimer2			 41	///< PPG 2/3/12/13/23/43  16-bit Free-run timer 2 (0 detection) / (compare clear)
...
#pragma	intvect	icu16_1_int				   45	///< 16-bit ICU 0 (fetching) / 16-bit ICU 1 (fetching)
...
#pragma	intvect	AD_Converter_interrupt_0_31    48	///< A/D converter 0/1/7/10/11/14/15/16/17/22/27/28/31
...
#pragma	intvect	Vectors_Isr_DefaultHandler 56	///< 32-bit ICU4 (fetching/measurement) Multi-function serial interface ch.11 (transmission completed)
#pragma	intvect	AD_Converter_interrupt_32_47 57	///< 32-bit ICU5 (fetching/measurement) A/D converter 32/34/35/37/38/40/41/42/43/44/45/46/47
...
/* 中断优先级管理，对应的ICR低5位=0x1F时，中断被disable */
void icr_manage(void)
{
    int i = 0;
    for(i =0; i<ICR_CHN; i++)
    {
        icr_set(i,ICR_PRI_MAX);		/* disable all interrupts */
    }
    /* 0为16号中断 */
    ...
    /*icr_set(5,22);*/		/* uart 0 发送中断 */
    icr_set(6,22); 	    //Multi-function serial interfacec ch.1 (reception completed),vector 22,UART1_rx_int
    ...
    //icr_set(23,26);		// free run timer
    //icr_set(24,26);		// free run timer
    //change priority for chord player
    icr_set(23,16);		// free run timer
    icr_set(24,16);		// free run timer
