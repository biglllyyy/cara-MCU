...
/*
 * Description : Initialize the phase-locked loop clock.
 * Parameter   : None
 * Retern value: None
 * Other	   : None
 * Attention   : The PLL stabilization wait time is necessary for 200us or more. 
 */
void hal_pll_init(void)
{
    ...
    while(1)
    {
        st = IO_CLKG.IO_CMONR.byte;		// Clock source Monitor Register 
                
        if((st&0x20) == 0x20)			// Main Clock Ready,CMONR.MCRDY=1
        {
            break;
        }
    }
    ...
    IO_CLKG.IO_PLLCR.hword = 0x0090;    // Pll clock OSc Wait 256usec
    IO_CCTL.IO_CCPSDIVR.byte = 0x11;    // PLL clock =  PLL macro clock 
    IO_CCTL.IO_CCPLLFBR.byte = 0x4F;    // PLL macro clock = MCLK/1x80   
    IO_CCTL.IO_CCSSFBR0.byte = 0x27;    // SSCG macro clock = MCLK/1x40x2 
    IO_CCTL.IO_CCSSFBR1.byte = 0x01;    // SSCG macro clock = MCLK/1x40x2  
    IO_CCTL.IO_CCCGRCR1.byte = 0xc1;    // Clock gear start step=1,increasing=4 
    IO_CCTL.IO_CCCGRCR2.byte = 0x01;    // Clock gear roop number=2
    IO_CCTL.IO_CCCGRCR0.byte = 0x01;    // Clock gear enable 
    									// Clock source SELect Register 
    IO_CLKG.IO_CSELR.byte = 0x60;   	// PLL valid */

    /* DIVide clock configuration */
	
    						/* Base clock setting */
    IO_DIVR0.byte = 0x00;   /* Divide ration of Baseclock */
    						/* external bus clock setting */
    IO_DIVR1.byte = 0x30;   /* Divide raiton of TCLK (B-ck/4) */
    						/* Peripheral clock setting */
    IO_DIVR2.byte = 0x30;   /* Divide ration of PCLK (B-ck/4) */

    /* PLL stabilization wait time( in 200[us] or more ) */
    while(1)
    {
        st = IO_CLKG.IO_CMONR.byte; /* Clock source MONitor Regiser */
									/* PLL Clock Ready */
        if((st&0x40) == 0x40)   	/* CMONR.PCRDY=1 */
        {
            break;
        }
    }
	
    /* Clock source change to PLL */
    									/* Clock source SELect Register */
    IO_CLKG.IO_CSELR.byte = 0x62;		/* set CKS --> PLL  */

    IO_CCTL.IO_CCCGRCR0.byte = 0x03;	/* Clock gear gear-up start */

    while(1)                        	/*------- gear up ok ´_ÕJ --------*/
    {
        st = IO_CCTL.IO_CCCGRCR0.byte;
        if((st&0xc0) == 0x80)           /* gear up ok(GRSTS=0x10) */
        {
            break;
        }
    }
