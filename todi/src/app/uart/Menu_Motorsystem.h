...
typedef struct {
    U8 engine_fuel_consumption;         //发动机油耗, 0.00 L/H
    U8 engine_water_temp;				//发动机水温, 0 ℃
    U8 engine_load;					    //发动机负荷, 0 %
    //int32_t urea_level;					//尿素液位, 0.0 %,通用帧已存在该变量
    U8 engine_target_throttle[4];			//发动机目标油门, 0.0 %
    U8 engine_actual_throttle[4];			//发动机实际油门, 0.0 %
    U8 engine_oil_pressure[4];			//发动机机油压力, Kpa
    U8 engine_inlet_temp;				//发动机进气温度, 0 ℃
