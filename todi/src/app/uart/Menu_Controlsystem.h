...
#define MENU_VCU_DATA_LENGTH (sizeof(MenuVcuControl))
#define MENU_VCU_FRAME_TYPE  VCU_INFO_TYPE
...
{
	U32 u32temp;
	memset(&s_control_para,0,MENU_VCU_DATA_LENGTH);
...
void send_vcu_system(void)
{
	U8 data[MENU_VCU_DATA_LENGTH + A20_MCU_DATA_LENTH];
	U32 parse_len;
    parse_len = app_uart_arm_send_parse(data,(void*)&vcu_msg,MENU_VCU_FRAME_TYPE,MENU_VCU_DATA_LENGTH);
	sent_data(UART_A20_CHN, data, parse_len); /* data sent */
}

void menu_vcu_system_Task(void)
{
	send_vcu_system();
}
