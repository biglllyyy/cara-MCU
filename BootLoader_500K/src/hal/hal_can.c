...
void hal_can_prepare(U8 nRecv, U8 nSend,can_msg_t*msg)
{
	...
	if (cntRecv || cntSend)
	{
		Console("Error: CAN already initialized\n");
		return;
	}
	cntRecv = nRecv;
	cntSend = nSend;
	...
	if (!can_msg)
	{
		Console("The Pointer of can_msg from mid_can is NULL\n");
		return;
	}
}
...
void can_receive_object_init(U8 chn,U8 num,CAN_ID_FORMAT_e format,U32 id,U8 dlc,U32 id_mask)
{
	...
	if(STAND_ID == format)
	{
        /*Arb Data*/
        /*MSK Data*/
        IO_CAN0.IF2MSK.word = 0xFFFC0000 | id_mask;				/*MXtd=1 MDir=1 res=1 MID28-MID18 all=1
														  MID17-MID0 all=0*/
...
void hal_can_sent(U8 chn,can_msg_t *can_msg)
{
    do
    {
        ...
        case 0:
            if(IO_CAN0.INTR.hword == 0x8000)		/* status int */
            {
				Console("IO_CAN0.INTR.hword = 0x8000\n");
                State_judge_0();
            }
            if(Error_State_0 == 0x01)
            {
				Console("Error_State_0 = 0x01\n");
                break;
            }
			IO_CAN0.IF1CMSK.hword = 0x00F0;
			IO_CAN0.IF1ARB.word  = 0xE0000000|(can_msg->id & 0x1FFFFFFF);  /* MsgVal:1,Xtd:1,Dir:1*/
