...
{
    memset(&DDR[0],0,sizeof(DDR)/sizeof(DDR[0]));
    ...
    WriteKeyProtectedRegister( (unsigned int)&PORTEN, 0xFD, BYTE_ACCESS);	//IO_INPUT enable
    //PMUCTLR_IOCT = 1;  //wake up from shutdown need add this

#if  1//P060作为IGN输入，既然放在HAL层，那么应该来说就是以后的仪表都不会更改了
    EPFR[11] &= MASK0 & MASK1;   // IG PIN No PPG IO_GENERAL_OUTPUT.　
    WriteKeyProtectedRegister((unsigned int)&EPFR11,EPFR[11],BYTE_ACCESS);
    EPFR[44] &= MASK5;
    WriteKeyProtectedRegister((unsigned int)&EPFR44,EPFR[44],BYTE_ACCESS); //IO_INPUT from 13_0
